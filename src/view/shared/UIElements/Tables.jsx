            ...
            {currentData.length > 0 ? (
              currentData.map((item) => (
                <tr className="hover:bg-sky-50 border border-sky-300 hover:border-sky-800 text-gray-700 " key={item.id}>
                  {columns.map((column) => (
                    <td className={`py-2 px-4 border-b border-sky-300 text-sm font-light ${column.class}`} key={column.key}>{item[column.key]}</td>
                  ))}
                </tr>
              ))
            ) : (
              <tr>
                <td colSpan={columns.length} className="text-center text-sky-700 bg-sky-100 border border-sky-300 py-2">
                  No data found
                </td>
              </tr>
            )}
